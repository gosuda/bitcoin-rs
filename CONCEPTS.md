...
`BITCOIN_RS_NETWORK`/`--network` atomically selects consensus rules and P2P bootstrap identity while preserving later low-level overrides. `NodeConfig::network_selection()` retains that resolved profile identity, while the internal consensus `Network` remains the Bitcoin consensus selector: `drynet4` keeps mainnet consensus with message start `eca5d404`, no Bitcoin DNS seeds, and `drynet4.drivechain.dev:8533`. Consensus extensions authorize activation from the retained profile identity, never from mutable transport settings such as P2P magic or fixed peers. Owner: `apply_network_selection` in `crates/node/src/config.rs`; layering is `ARCH-05` in `docs/contracts/architecture.md`.
