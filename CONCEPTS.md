...
A block identity is the set of transaction IDs used by native apply; see the normative hashing and kernel-boundary rules in [VAL-01](docs/contracts/validation-default.md#val-01-library-default-is-native-kernel-is-an-oracle-feature). `KernelBlock` remains available under `--features kernel` as a comparison oracle, not a substitute for production apply.
