...
A key that says which producer wrote a row, not merely where it is. Funding, spending, and txid keys are an 8-byte prefix plus a big-endian height, so two same-height blocks sharing a script collide and a second rollback of the first would delete the second's rows. The block-header row (keyed by the 32-byte double-SHA256 of the header) is identity-bearing; checking it before deleting stands in for rekeying the other families.
