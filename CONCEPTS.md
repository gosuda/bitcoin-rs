...
Apply-path transaction identity uses the validation contract’s native hashing rule; see [`VAL-01`](docs/contracts/validation-default.md#val-01). `KernelBlock` remains available under `--features kernel` so the Core parse can be compared, not substituted.
