...
`BITCOIN_RS_NETWORK`/`--network` atomically selects consensus rules and P2P bootstrap identity while preserving later low-level overrides. The internal consensus `Network` remains the consensus selector: `drynet4` keeps mainnet consensus with message start `eca5d404`, no Bitcoin DNS seeds, and `drynet4.drivechain.dev:8533`. Owner: the node option table in `crates/node/src/options.rs`, which declares the selection's spellings, and `NetworkProfile::for_selection` in `crates/node/src/config.rs`, which fills the profile's fields once after every layer has been folded. Layering is `ARCH-05` in `docs/contracts/architecture.md`.
