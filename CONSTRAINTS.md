...
| Solver | `SMT_SOLVER=z3`; `JVM_ARGS` default 4096m |
...
| --- | --- | --- | --- | --- | --- | --- | --- |
| ChainAdmission | UNMEASURED | UNMEASURED | UNMEASURED | 128 | UNMEASURED | UNMEASURED | UNMEASURED |
| PeerLeases | UNMEASURED | UNMEASURED | UNMEASURED | 128 | UNMEASURED | UNMEASURED | UNMEASURED |
| ProjectionMining | UNMEASURED | UNMEASURED | UNMEASURED | 128 | UNMEASURED | UNMEASURED | UNMEASURED |
...
| CL-09 | Immutable pins | `ReferenceSet` records independent source, binary, and corpus digests; pin mempool `v3.3.1`, bitcoin-wallet `885198873` at `2fe2af12c721bdf2cd4af7801146bda40fa15429`, `bip324==0.11.0`, toolchain `1.95.0`, and `Cargo.lock` SHA-256. Candidate-derived expected answers are not independent references. | `cargo test --locked -p bitcoin-rs --no-default-features --features fjall --test overhaul_reference_set -- --nocapture` | G0 and every gate | Project-wide evidence and reference identity; T00, T28, T33, T34 | Required true; T00/T28/T33/T34, Verification 0, FP-03 | UNMEASURED | UNMEASURED | UNMEASURED | UNMEASURED |
| CL-10 | Actual external consumers | Unmodified mempool backend plus frontend, actual `btctxbuilder` over public transports, and external miner over GBT each require their owner-gate evidence. In-process shortcuts do not prove the consumer contract. | `cargo test --locked -p bitcoin-rs --no-default-features --features fjall --test overhaul_mempool_consumer -- --nocapture` | G8, G9; T02 existing-I/O baseline | Diff: external-consumer contracts; T33, T34, T36 | Required true; R024/R035/R043, Verification 4 | UNMEASURED | UNMEASURED | UNMEASURED | UNMEASURED |
...
| CL-21 | No unapproved surface | A public boundary outside Approval effects stops the cut pending separate human approval. No unapproved limit, config, or crate beyond approved `crates/chainstate`; `bip324` is not default and optional C is transport-only. No writes to operator data; test and replay writes are confined to disposable isolated fixtures. | `cargo test --locked -p bitcoin-rs --no-default-features --features fjall --test overhaul_ownership -- --nocapture` | Every cut | Diff: all changed boundaries, dependencies, configuration and I/O | Required true; Approval effects, T28 | UNMEASURED | UNMEASURED | UNMEASURED | UNMEASURED |
| CL-22 | Version/lock discipline | Approved workspace 0.4.0 to 0.5.0 release change and `Cargo.lock` update are atomic. Schema and projection versions stay owner-local; disposable projections never dictate authoritative schema. Commands use `--locked`; `deny.toml` skips exactly one `bitcoin_hashes` with `bip324` its sole reverse dependency. | `cargo test --locked -p bitcoin-rs --no-default-features --features fjall --test overhaul_release_profiles -- --nocapture` | G7, G11 | Diff: owner-local versions; project-wide release, lock, and dependency closure; T28, T40 | Required true; `docs/policies/source-compatibility.md` section 4, T28 | UNMEASURED | UNMEASURED | UNMEASURED | UNMEASURED |
