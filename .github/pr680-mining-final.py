...
for text in ['use bitcoin_rs_mining::MiningControl;\n', '    mining_control: Arc<dyn MiningControl>,\n', '        mining_control,\n', '                    self.mining_control.publish_generation();\n']:
    assert text in production, repr(text)
    production = production.replace(text, '')
...
tests = re.sub(r'\.is_orphan\(&([a-z_]+)\)', r'.have_tx(Hash256::from(\1), false)', tests)
tests = re.sub(r'\.is_rejected\(Hash256::from\(([a-z_]+)\)\)', r'.have_tx(Hash256::from(\1), true)', tests)
assert '.is_orphan(' not in tests and '.is_rejected(' not in tests
# Retain every existing named unit regression when upgrading its observer wiring.
names = lambda text: set(re.findall(r'#\[test\]\s*fn\s+(\w+)', text))
assert names(s) <= names(tests), names(s) - names(tests)
...
''' + s[b:]
old = 'let mining_control: Arc<dyn MiningControl> = Arc::<RecordingMining>::clone(&mining);'
assert old in s
s = s.replace(old, 'let wake: Arc<dyn MempoolSequenceWake> = mining.clone();\n        state.mining_generation_signal().attach_sequence_wake(&wake);')
