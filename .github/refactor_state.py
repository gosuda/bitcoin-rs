...
storage=storage.replace('struct JournalBootstrap {','pub(super) struct JournalBootstrap {',1)
for fld in ['open_existing','base_generation','height','block_hash','prev_hash','chain_tx_count','config']:
    storage=storage.replace(f'    {fld}:',f'    pub(super) {fld}:',1)
...
bootstrap=bootstrap.replace('struct InitialChainstate {','pub(super) struct InitialChainstate {',1)
for fld in ['utxo','coin_stats','tree','applied_tip','chain_tx_count','resume_source','journal_bootstrap']:
    bootstrap=bootstrap.replace(f'    {fld}:',f'    pub(super) {fld}:',1)
...
tx=tx.replace('struct TxIndexSpawn {','pub(super) struct TxIndexSpawn {',1)
for fld in ['spec','generation','block_source','body_source','wake_rx','recovery_reporter']:
    tx=tx.replace(f'    {fld}:',f'    pub(super) {fld}:',1)
