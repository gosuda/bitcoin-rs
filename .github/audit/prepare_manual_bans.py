"""Apply a hash-guarded manual-ban regression/fix candidate to a clean checkout."""
...
def replace(path, old, new):
    p = Path(path)
    text = p.read_text()
    assert text.count(old) == 1, (path, 'replacement count', text.count(old), old[:80])
    p.write_text(text.replace(old, new, 1))


def append(path, text):
    p = Path(path)
    assert 'mod manual_ban_tests' not in p.read_text(), path
...
mod manual_ban_tests {
    ...
    #[test]
    fn manual_ban_service_revokes_registered_sessions()
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert!(leases[0].0.is_cancelled(), "manual ban left a matching live lease active");
    ...
    #[test]
    fn manual_ban_expired_entry_does_not_disconnect_current_peers()
    -> Result<(), Box<dyn std::error::Error>> {
        let service = P2pService::new(P2pServiceConfig::default(), Arc::new(AtomicBool::new(false)));
...
''')
    append('crates/rpc/src/handlers/network.rs', r'''
...
mod manual_ban_tests {
    ...
    #[test]
    fn manual_ban_rpc_disconnects_only_the_selected_subnet()
    -> Result<(), Box<dyn std::error::Error>> {
    ...
    #[test]
    fn manual_ban_timestamp_overflow_cannot_become_a_permanent_ban()
    -> Result<(), RpcError> {
        let ctx = Arc::new(Context::new());
...
mod manual_ban_tests {
    ...
    #[test]
    fn manual_ban_is_rechecked_after_accept_before_handshake_registration()
    -> Result<(), Box<dyn std::error::Error>> {
        let listener = TcpListener::bind("127.0.0.1:0")?;
        let client = TcpStream::connect(listener.local_addr()?)?;
        let (server, peer_addr) = listener.accept()?;
        // Close the write half so the old path terminates on EOF, not a timeout.
        ...
        // The socket has been accepted, but its handshake worker has not registered.
        service.set_ban(crate::BannedSubnet {
...
''')
    ...
    replace('crates/p2p/src/listener.rs', '    fn is_session_cancelled(&self) -> bool {\n', '''    fn register_connection(
        ...
        // Keep the read guard until registration completes; a pre-connect or
        // accept-loop check alone leaves a gap before the worker registers.
...
''', '''            reason: reason.to_owned(),
        ...
        let disconnected = crate::service::apply_manual_ban(
            &mut banned,
            &self.peer_table,
            entry,
            now,
        ...
        drop(banned);
''')
    replace('crates/rpc/src/handlers/network.rs', '''fn ban_until(now: SystemTime, bantime: u64, absolute: bool) -> Option<SystemTime> {
    if absolute {
...
''', '''            let entry = BannedSubnet {
                subnet,
                banned_until: Some(ban_until(now, bantime, absolute)?),
                ban_created: now,
                reason: "manual".to_owned(),
            };
            bitcoin_rs_p2p::apply_manual_ban(
                &mut ctx.banned.write(),
                &ctx.peer_table,
                entry,
                now,
            );
