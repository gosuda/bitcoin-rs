...
model=localize(sl(748,1021)).replace('    chainstate: &\'a Chainstate,','    pub(super) chainstate: &\'a Chainstate,').replace('    proof: ChainChangeProof<\'a>,','    pub(super) proof: ChainChangeProof<\'a>,')
...
window=localize(sl(1879,2386))
write('apply/window.rs','//! Bounded consecutive-block apply windows and failure classification.\n\nuse super::*;\nuse super::admission::*;\nuse super::validation::*;\n\n'+window)
validation=localize(sl(2388,4124))
validation=validation.replace('''pub(super) struct BlockValidationContext {
    hash: Hash256,
    parent: Hash256,
    height: u32,
    flags: bitcoin_rs_script::VerifyFlags,
    locktime_cutoff: u32,
}''','''pub(super) struct BlockValidationContext {
    hash: Hash256,
    parent: Hash256,
    pub(super) height: u32,
    pub(super) flags: bitcoin_rs_script::VerifyFlags,
    pub(super) locktime_cutoff: u32,
}''')
validation=validation.replace('''pub(super) struct PreparedApply<'b> {
    kernel_block: bitcoin_rs_consensus::kernel::KernelBlock,''','''pub(super) struct PreparedApply<'b> {
    pub(super) kernel_block: bitcoin_rs_consensus::kernel::KernelBlock,''')
validation=validation.replace('    view: bitcoin_rs_consensus::BlockView<\'b>,','    pub(super) view: bitcoin_rs_consensus::BlockView<\'b>,',1).replace('    tx_plan: BlockTxPlan,','    pub(super) tx_plan: BlockTxPlan,',1).replace('    resolved: Arc<ResolvedUtxoView>,','    pub(super) resolved: Arc<ResolvedUtxoView>,',1)
validation=validation.replace('    fn same_block_spent_set(&self)', '    pub(super) fn same_block_spent_set(&self)',1).replace('    fn resolve<S: crate::window_overlay::OutputSource + ?Sized>(', '    pub(super) fn resolve<S: crate::window_overlay::OutputSource + ?Sized>(',1)
