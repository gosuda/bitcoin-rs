...
#   scripts/check-feature-matrix.sh pure     # fjall/redb/zmq, no kernel/rocksdb
...
# illegal singles (zmq-only, kernel-only on node).
...
while IFS=$'\t' read -r name package lane args; do
  ...
  log "${name}: cargo check --locked -p ${package} ${args}"
  # shellcheck disable=SC2086
  cargo check --locked -p "${package}" ${args}
  checked=$((checked + 1))
done < "${MATRIX}"
