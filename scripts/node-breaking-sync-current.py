...
if not NODE_STAGE.exists():
    raise SystemExit("node block stager owner is already absent")
...
src = src.replace(
    "bitcoin_rs_p2p::download_window::PENDING_BLOCK_BYTE_ESTIMATE",
    "crate::download_window::PENDING_BLOCK_BYTE_ESTIMATE",
)
...
for old, new in replacements.items():
    if old not in src:
        raise SystemExit(f"expected stage fragment missing: {old[:100]!r}")
...
lib = lib.replace(
    needle,
    "/// Out-of-order block-body staging and staging budgets.\npub mod block_stager;\n" + needle,
    1,
)
anchor = "pub use chain_query::ActiveChainQuery;\n"
if anchor not in lib:
    raise SystemExit("p2p re-export anchor missing")
lib = lib.replace(
    anchor,
    anchor + "pub use block_stager::{BlockStager, DrainedBlock, DroppedBlock, StagedBlock};\n",
    1,
)
P2P_LIB.write_text(lib)

sync = NODE_SYNC.read_text()
module_anchor = "mod stage;\n\n"
if module_anchor not in sync:
    raise SystemExit("node stage module declaration missing")
sync = sync.replace(module_anchor, "", 1)
old_import = (
    "use bitcoin_rs_p2p::{InboundBlock, InboundHeaders, Message, PeerInfo, PeerSource, PeerTable};"
)
new_import = """use bitcoin_rs_p2p::{
    BlockStager, DrainedBlock, InboundBlock, InboundHeaders, Message, PeerInfo, PeerSource, PeerTable,
    StagedBlock,
};"""
if old_import not in sync:
    raise SystemExit("current node p2p import anchor changed")
sync = sync.replace(old_import, new_import, 1)
legacy_import = "\nuse self::stage::{BlockStager, DrainedBlock, StagedBlock};\n"
if legacy_import not in sync:
    raise SystemExit("legacy stage import missing")
sync = sync.replace(legacy_import, "\n", 1)
NODE_SYNC.write_text(sync)

# Hard-cut contract: node cannot retain the old owner or an alias back to it.
if NODE_STAGE.exists():
    raise SystemExit("legacy node stager still exists")
for path in (ROOT / "crates/node").rglob("*.rs"):
    text = path.read_text()
    if "sync::stage" in text or "self::stage" in text or "mod stage;" in text:
        raise SystemExit(f"legacy node staging path remains in {path}")
