...
`--fast-sync` (also `BITCOIN_RS_FAST_SYNC` or `fast_sync` in TOML) relaxes the block-download policy: the node targets 32 outbound peers instead of 8 and begins fan-out as soon as two eligible peers are connected instead of eight. The 256-block download window is divided dynamically across the currently eligible peers, with a minimum stripe of 8 blocks per peer: two eligible peers receive 128 blocks each, and the 8-block floor is reached at the 32-peer target. Consensus validation is unchanged. The mode is opt-in and its throughput has not been measured against the default.
