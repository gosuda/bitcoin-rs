...
| [mining-selection.md](mining-selection.md) | `MIN-01` | Snapshot-based mining selection consumes supplied topology without a second DAG validation pass | `crates/mining` candidate assembly | `crates/mining/tests/policy_pareto.rs` test `missing_ancestors_fail_assembly_without_rechecking_the_dag` |
| [external-api.md](external-api.md) | `API-01`–`API-09` | Pointer: JSON-RPC/REST/ZMQ manifest, generated reference, error code mappings, query budgeting, solo-mining generate, `getnetworkhashps` snapshot behavior, BIP22/BIP23 template extras, mainnet GBT operational gates, and `submitheader` | RPC/REST/ZMQ clients; `tools/bip300301-enforcer` | `crates/rpc/tests/manifest_coverage.rs` tests `rpc_rows_and_the_live_registry_agree_both_ways`, `generated_reference_matches_checked_in`; `crates/rpc/src/handlers/mining.rs` generate, GBT extra, GBT gate, and `submitheader` tests; `crates/node/tests/mining.rs` generate, GBT extra, and `submitheader` tests; `crates/node/src/mining.rs` test `hash_ps_at_rejects_a_height_the_tip_cannot_resolve`; `crates/node/tests/mining.rs` test `network_hash_ps_rejects_core_invalid_windows` |
