...
| [muhash-rpc.md](muhash-rpc.md) | `MRPC-01`–`MRPC-04` | Production `gettxoutsetinfo` arity, attested-child ownership of the timed RPC connection, workspace copies of pinned arm configs, and coherent UTXO/tip query reads | `crates/rpc` `gettxoutsetinfo`; `tools/benchmark-campaign/muhash_rpc.py` | `crates/rpc` `gettxoutsetinfo_rejects_trailing_parameters`; `tools/benchmark-campaign/test_muhash_rpc.py` (`python3.13 tools/benchmark-campaign/test_muhash_rpc.py`) |
