...
| [indexing.md](indexing.md) | `IDX-01`–`IDX-10` | Capability gating, coherent reads, reset/rebuild, reorg reconciliation, worker scheduling, and sortable ScriptIndex height keys | txindex worker recovery/query/lifecycle/scheduling suites; `crates/index/tests/be_order.rs`; `crates/index/tests/index_roundtrip.rs` `format_3_open_resets_historical_keys_and_keeps_live`, `format_4_open_resets_historical_keys_and_keeps_live`; RPC capability tests
