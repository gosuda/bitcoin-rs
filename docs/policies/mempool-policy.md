...
This policy applies to the transaction-acceptance surface of `bitcoin-rs-mempool` (`crates/mempool`: `standardness.rs`, `policy.rs`, `rbf.rs`, `eviction.rs`) and its two admission outlets — the pool mutation API and the RPC handlers `sendrawtransaction` / `testmempoolaccept` (`crates/rpc/src/handlers/tx.rs`). It covers relay policy and the admission-time consensus checks the mempool seam runs (script verification under `STANDARD` flags, BIP113 finality). Consensus script semantics and sighash details live in `docs/policies/core-compatibility.md` and the consensus tests; this document owns only the admission-outlet contract that those checks are performed and quoted identically.
...
| Script verification | ATMP `PolicyScriptChecks` under `STANDARD_SCRIPT_VERIFY_FLAGS`; `testmempoolaccept` and `sendrawtransaction` quote the same reject class | Same: the mempool acceptance seam (`evaluate_one`) runs `verify_transaction` under `VerifyFlags::STANDARD` after policy, so both RPC outlets reject unsigned, script-invalid, duplicate-input, and overspending transactions | implemented | `package_acceptance_rejects_an_unsigned_p2wpkh_spend`, `sendrawtransaction_and_testmempoolaccept_reject_unsigned_p2wpkh`, `unsigned_p2wpkh_spend_rejects_on_both_rpc_outlets` |
| Non-final locktime | `CheckFinalTxAtTip` against next-block height and applied-tip MTP (BIP113) | Same: the acceptance seam evaluates finality at `applied_tip + 1` with the applied-tip MTP; a timestamp lock that is final only under a header tip that has run ahead is rejected | implemented | `package_acceptance_rejects_a_non_final_locktime`, `sendrawtransaction_rejects_tx_final_only_under_header_tip_mtp`, `testmempoolaccept_rejects_tx_final_only_under_header_tip_mtp` |
...
| Script verification | `ScriptVerify` | transaction rejected (−26), message contains `script-verify-flag-failed` | `reject-reason: "script-verify-flag-failed"` | Core: `mandatory-script-verify-flag-failed` / `script-verify-flag-failed` |
| Non-final locktime | `NonFinal` | transaction rejected (−26), message contains `non-final` | `reject-reason: "non-final"` | Core: `non-final` |
...
2. **BIP68 / sequence-lock policy checks are absent.** Core rejects non-final (`non-BIP68-final`) transactions at admission; the mempool seam defines the `NonBip68Final` fact but nothing produces it yet. Absolute locktime / BIP113 finality **is** enforced. No fixture claims BIP68 relative-lock policy.
...
4. **Error codes.** Policy rejections surface as JSON-RPC internal errors (−32603) with the class text in the message, not Core's transaction error codes (§4). Numeric-code alignment is owned by the RPC compatibility manifest.
