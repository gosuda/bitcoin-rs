...
1. **Preview rows do not short-circuit after a package failure.** The gateway preview evaluates every submitted row independently and resolves inputs from earlier offered outputs. It preserves the supported RPC row contract, not Core's package failure short-circuit or atomic package admission. The policy-only `evaluate_package_acceptance` helper retains its separate short-circuit contract.
2. **Generic script rejection text.** Preview and submission both verify scripts, but render their shared rejection as `script-verify-flag-failed` and `consensus-verification-failed`, respectively. Core exposes finer script-failure detail. No exact Core script-error-string parity is claimed.
