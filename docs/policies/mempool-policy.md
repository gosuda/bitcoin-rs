...
| RBF signaling | No opt-in signaling requirement (full RBF) | Same: the mempool owner evaluates replacements independently of sequence signaling; `getmempoolinfo.fullrbf` is true | implemented | `replacement_signaling_matches_pinned_core` (real-process acceptance, insufficient-fee rejection, and policy reporting), `replacements_accept_mixed_signaling_conflicts`, `nonsignaling_replacements_agree_on_both_rpcs` |
...
| RBF rule 4 | Replacement fee ≥ evicted fees + incremental relay fee × vsize | Same | implemented | `rbf_replacement_sweeps_nonsignaling_conflicts_and_descendants` (boundary), `bip125_rule4_replacement_must_pay_incremental_relay_fee_on_both_rpcs` |
| RBF rule 5 | At most 100 distinct clusters containing direct conflicts | At most 100 evicted transactions, including descendants (`max_replacement_evictions = 100`) | deviation | `rbf_bip125.rs::retained_replacement_rules_are_enforced` (rule-5 case), `bip125_rule5_too_many_evicted_descendants_reject_on_both_rpcs` (current behavior; Core process comparison remains #639) |
...
| RBF rules | `Replacement(RbfError)` | `TxRejected` (−26), containing `BIP125 rule N` | `reject-reason` carries the same text | Historical rule 2 remains a bitcoin-rs rejection after Core removed it; remaining replacement checks have implementation-specific strings |
...
5. **Remaining replacement policy differences (#639).** bitcoin-rs retains
   the new-unconfirmed-input restriction, a transaction-count eviction limit,
   and direct-conflict fee-rate comparison. Core 31.1 removes the input
   restriction, counts conflicting clusters, and requires complete
   feerate-diagram improvement. These differences are source comparisons
   backed by current-behavior fixtures; their independent process scenarios
   remain open. The RPC compatibility manifest therefore keeps preview and
   submission as `deviation`. Signal-independent replacement, insufficient-fee
   rejection, and `fullrbf` reporting have their own real-process comparison.
...
- **Production RBF signaling comparison**:
  ...
  submission with nonsignaling and opt-in originals: higher-fee replacements
  succeed and underpaying replacements reject on both nodes. Preview cannot
  change public membership or sequence; rejected submission preserves both,
  while successful replacement removes the original. Both nodes report
  `fullrbf = true`. Launch identities and RPC replies remain in
  `target/process-harness/run-*/`. Other replacement/package rules retain the
