...
| Duplicate submission | A tx still in the mempool succeeds without re-admission; a tx no longer in the mempool is processed again by `BroadcastTransaction` ([Core 31.1](https://github.com/bitcoin/bitcoin/blob/v31.1/src/node/transaction.cpp#L120-L169)) | Same distinction: `sendrawtransaction` returns the txid for a pool hit, but a retained transaction that was evicted is re-admitted and subject to current policy; preview reports `txn-already-in-mempool` only for a pool hit | implemented | `transaction_methods.rs::sendrawtransaction_idempotent_for_already_in_mempool` (pool hit), `transaction_methods.rs::pol01_sendrawtransaction_readmits_a_transaction_evicted_from_the_mempool` (evicted resubmission) |
