...
facts. Sigop counting is owned by
`bitcoin_rs_script::sigops::count_tx_sigop_cost`, shared with consensus.
Submission, preview, and reorg preparation consume those facts without a
second validation engine. Legacy and P2SH sigops carry the witness scale
factor; witness-v0 sigops carry unit cost, following
...
| Standardness: tx version | `IsStandardTx`: versions 1–2 standard (3 under TRUC policy) | Versions 1–2 only; v3 rejected at the version gate (Core accepts v3 under TRUC; the TRUC package policy is not implemented — ledger §5.4) | deviation | `rejects_version_three_while_truc_policy_is_absent` (`standardness.rs` unit test), `testmempoolaccept_reports_a_policy_verdict_per_row` (RPC row: `non-standard transaction version`) |
...
2. **Preview omits policy script checks.** `testmempoolaccept` uses `evaluate_package_acceptance_all`, which does not run `verify_transaction`. Gateway admission (`MempoolGateway::admit_transaction`) verifies scripts under `VerifyFlags::STANDARD` before pool mutation. Preview approval is not a script-validity guarantee. Pinned by `admit_transaction_rejects_a_script_invalid_input`.
