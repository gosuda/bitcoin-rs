...
- **Live lane (cut)**: `scripts/run-p2p-core-interop.sh` drives a real Bitcoin Core 31.1 (regtest, installed by `scripts/install-bitcoind.sh`) plus a bitcoin-rs node through the initial sync, mines extra blocks after the handshake to prove the node follows Core's announcements while connected (bitcoin-rs itself sends no proactive block announcements; see the deviation ledger), diffs `getblockcount` / `getbestblockhash` / `getblockchaininfo.{chain,blocks}`, records Core's own `getpeerinfo` view of us (services bits, subver) into an evidence JSON, and runs the `#[ignore]`d verifier `crates/p2p/tests/core_interop_live.rs`. The main workflow `core-differential` job runs this lane.
