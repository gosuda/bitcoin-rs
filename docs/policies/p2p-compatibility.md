...
| `getblocktxn` | served | BIP152. Answered with `blocktxn` from the active chain under the same body-availability and headroom rules as full-block `getdata`, while the block is within 10 of the active tip; a deeper request is answered with the whole witness-bearing `block`, as Core does (`net_processing.cpp:4590-4624`). The peer's recorded serving profile selects stripped transactions for v1 and witness transactions for v2, matching its `cmpctblock` prefills. An unknown recorded version maps to v1; without negotiation the existing full-witness `blocktxn` response remains available while compact inventory is `notfound`. An empty or non-increasing index list, or an index past the end of the block, is a protocol disconnect (Core scores misbehavior). |
