...
The decoder types exactly the commands in `crates/p2p/src/compat.rs::COMMANDS` (**36**). That table is the authority for names and status; this section is the Core-comparison commentary and is checked for set equality of command names. Statuses: *negotiated* (sent and processed in the handshake), *served* (answered with protocol data), *sink* (decoded and forwarded into the node), *ignored* (decoded, FSM-accepted, no response), *legacy* (decoded for corpus/legacy tolerance only).
...
| `getdata` | served | Block inventory is served from the active chain; misses resolve to one `notfound`. Transaction inventory is served when a `TxInventory` handle is wired (witness `tx` bodies, BIP339 `WTx` by wtxid). The production listener still dispatches without that handle, so inbound `getdata` for transactions is `notfound` until #323. Bound: 50 000 vectors. |
...
| `tx` | sink (incomplete) | Decoded and FSM-accepted. Production `Message::Tx` is not delivered into the mempool: the listener has no inbound-tx sink and `spawn_tx_ingress_consumer` is test-only (#323). No response, no disconnect. |
...
9. **Inbound transaction relay into the mempool** incomplete (§5 `tx` row); protocol-level acceptance is proven, production mempool delivery is #323.
