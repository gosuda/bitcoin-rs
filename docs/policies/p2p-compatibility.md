...
| `getblocktxn` | served | BIP152. Answered with `blocktxn` from the active chain under the same body-availability and headroom rules as full-block `getdata`, while the block is within 10 of the active tip; a deeper request is answered with the whole witness-bearing `block`, as Core does (`net_processing.cpp:4590-4624`) — the depth check runs before the past-the-end index check there too, so an index past the end of a deep block still receives the full block rather than an error. On the `blocktxn` path an index past the end of the block is a protocol disconnect (Core scores misbehavior). An empty or non-increasing index list is rejected at the inbound boundary for every `getblocktxn`, regardless of depth. |
