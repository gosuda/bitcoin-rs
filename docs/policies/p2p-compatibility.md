...
| `getblocktxn` | served | BIP152. Answered with `blocktxn` from the active chain under the same body-availability and headroom rules as full-block `getdata`, while the block is within 10 of the active tip; a deeper request is answered with the whole witness-bearing `block`, as Core does (`net_processing.cpp:4590-4624`) — the depth check runs before index validation there too, so out-of-range indexes on a deep block still receive the full block rather than an error. On the `blocktxn` path an empty or non-increasing index list, or an index past the end of the block, is a protocol disconnect (Core scores misbehavior). |
