...
| `estimatesmartfee` | 0.4.0 | See docs/contracts/external-api.md#API-23 for estimate_mode and conf_target validation. The estimate itself comes from this node's mempool horizon, not Core's confirmation estimator (crates/rpc/src/handlers/util.rs). |
