...
- **Supported** - implemented and differentially verified against the pinned reference. Nothing qualifies while `[reference].differential_harness` is false.
- **Deviation** - shipped with a recorded difference from Core; notes cite the source file.
- **Implemented (unverified)** - shipped; nothing has compared it to the pinned reference.
- **Extension** - bitcoin-rs-specific surface with no Core counterpart.
- **Disabled** - shipped surface refused at the parameter level with a stable error. Reserved; no row carries it today.
