...
| `generateblock` | 0.4.0 | Assembles and solves one block paying an address or descriptor from the listed mempool txids or raw txs in that order. See docs/contracts/external-api.md#API-27 for transaction parsing, #API-29 for invalid-output errors, #API-30 for pre-solve validity, and #API-31 for descriptor expansion errors. |
...
| `estimatesmartfee` | 0.4.0 | See docs/contracts/external-api.md#API-26 for estimate_mode and conf_target validation. The estimate itself comes from this node's mempool horizon, not Core's confirmation estimator (crates/rpc/src/handlers/util.rs). |
