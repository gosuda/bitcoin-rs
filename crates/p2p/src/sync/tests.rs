...
/// and block-at-a-time applied-tip advance, with one-shot scripted commit
/// failures and merkle-mismatch permanent invalidation in place of node's
/// `Chainstate` apply semantics.
struct TestChain {
    network: Network,
...
impl SyncChain for TestChain {
    ...
    fn commit_window(
        ...
        for block in blocks {
            ...
            // A non-empty body whose transactions no longer match the
            // header's merkle root is a permanent consensus failure; the
            // node-side implementation invalidates the subtree, so mirror
            // that here. Empty bodies are header-chain fixtures.
            if !block.txs.is_empty() && merkle_root(&block.txs) != block.header.merkle_root {
                let invalidated = tree
                    .invalidate_subtree(node_id)
                    .map(Vec::into_boxed_slice)
                    .unwrap_or_default();
                // The body merkle root disagrees with its admitted header.
                return Err(WindowCommitError {
                    applied,
                    disposition: WindowCommitDisposition::Permanent,
                    invalidated,
                    source: Box::new(std::io::Error::other("merkle root mismatch")),
                });
            }
    ...
    fn switch_to_branch(
        ...
        _connected_body: &mut dyn FnMut(Hash256),
