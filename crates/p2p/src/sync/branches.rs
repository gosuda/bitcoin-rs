...
impl BlockSync {
    ...
    pub fn switch_branch_if_outweighed(&self) {
        ...
        let outcome = self.chain.switch_to_branch(
            target,
            &mut |hash| self.frontier_state.lock().stager.staged_body(hash),
        ...
        match outcome {
            ...
            Err(BranchSwitchError::ConnectFailed {
                ...
                if disposition == WindowCommitDisposition::BodyMutated {
                    ...
                    frontier_state.stager.retire_applied(&hash);
                    frontier_state.window.drop_for_retry(&hash);
                }
    ...
    pub fn retire_applied_reorg_body(&self, hash: Hash256) {
        let mut frontier_state = self.frontier_state.lock();
        frontier_state.window.mark_received_applied(&hash);
        frontier_state.stager.retire_applied(&hash);
    }

    /// Frees every bounded download slot held by an invalidated hash under
    /// one `frontier_state` acquisition.
    #[doc(hidden)]
