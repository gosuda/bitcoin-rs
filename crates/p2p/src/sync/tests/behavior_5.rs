...
fn fatal_settlement_halts_further_apply_attempts() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let staged = sync.frontier_state.lock().stager.received_len();
    assert!(
        staged > 0,
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(
        fixture.sync.frontier_state.lock().stager.received_len() <= max_received_blocks,
        "block stager must enforce received block count budget"
    );
    assert!(
        fixture.sync.frontier_state.lock().window.received_len() <= max_received_blocks,
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    sync.frontier_state.lock().header_request = Some(super::super::PendingHeaderRequest {
        source: predecessor,
