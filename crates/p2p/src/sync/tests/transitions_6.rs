...
fn window_failure_applies_prefix_and_restores_suffix() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let restored = fixture.sync.frontier_state.lock().stager.received_len();
    assert_eq!(
        restored, 0,
...
-> Result<(), Box<dyn std::error::Error>> {
    ...
    sync.tick();
    assert!(sync.frontier_state.lock().header_request.is_some());
    ...
    sync.reconcile_peer_sessions();
    assert!(sync.frontier_state.lock().header_request.is_none());
    Ok(())
}
