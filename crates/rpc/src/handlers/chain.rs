...
pub(crate) fn getchaintips(ctx: &Arc<Context>, params: &Value) -> Result<Value, RpcError> {
    ...
    let active_tip = ctx.chain.applied_tip.load_full();
    ...
    ctx.chain.with_stable_chainstate(|| {
        ...
        let window_block_count = match array.first().filter(|value| !value.is_null()) {
            ...
            Some(value) => {
                let nblocks = value
                    .as_i64()
                    .ok_or(RpcError::InvalidType("nblocks must be a number".to_owned()))?;
...
pub(crate) fn getblockstats(ctx: &Arc<Context>, params: &Value) -> Result<Value, RpcError> {
    ...
    let subsidy_sat = bitcoin_rs_consensus::block_subsidy(
        height,
...
fn resolve_per_tx_fees(ctx: &Context, block: &Block) -> Result<Vec<(u64, u64)>, TxQueryError> {
    let Some(derived_index) = ctx.indexes.derived_index.as_ref() else {
...
pub(crate) fn invalidateblock(ctx: &Arc<Context>, params: &Value) -> Result<Value, RpcError> {
    ...
    let control = ctx
        ...
        .chain_control
        .as_ref()
        .ok_or(RpcError::MethodNotFound("invalidateblock".to_owned()))?;
...
pub(crate) fn verifychain(ctx: &Arc<Context>, params: &Value) -> Result<Value, RpcError> {
    ...
    let tree = ctx.chain.block_tree.read();
    let Some(applied) = ctx.chain.applied_tip.load_full() else {
        return typed_to_sonic(&v31::VerifyChain(true));
    };
...
pub(crate) fn gettxoutsetinfo(ctx: &Arc<Context>, params: &Value) -> Result<Value, RpcError> {
    ...
    let hash_type = match array.and_then(|values| values.first()) {
        ...
        Some(value) => value
            .as_str()
            .ok_or(RpcError::InvalidType("hash_type must be a string".to_owned()))?,
    };
    ...
    // The tip capture happens inside the stable view: a connect commits its
    // UTXOs before publishing the tip, so a tip read here can never describe
    // a block the scan has not seen.
    let (stats, txouts, transactions, set_hash, scan_height, scan_hash) =
        ctx.chain.utxo.with_stable_view(|stable| {
            ...
            let scan_hash = tip.map_or_else(
                || ctx.chain.chain_network.genesis_block_hash(),
                |tip| tip.hash,
            );
            let stats = bitcoin_rs_utxo::stats::scan_coin_stats(stable, scan_height, want_muhash)
                .map_err(|err| RpcError::Internal(err.to_string()))?;
...
fn scanobjects_param(params: &Value) -> Result<&sonic_rs::Array, RpcError> {
    ...
    let scanobjects = scanobjects
        .as_array()
        .ok_or(RpcError::InvalidType("scanobjects must be an array".to_owned()))?;
...
fn scanobject_descriptor(scanobject: &Value) -> Result<&str, RpcError> {
    ...
    let descriptor = descriptor
        .as_str()
        .ok_or(RpcError::InvalidType("scan object desc must be a string".to_owned()))?;
...
fn getblock_verbosity(params: &Value) -> Result<u64, RpcError> {
    ...
    Err(RpcError::InvalidType("verbosity must be number or boolean".to_owned()))
}
...
fn parse_hash(value: &str) -> Result<Hash256, RpcError> {
    ...
    Hash256::from_str(value).map_err(|_| {
        RpcError::InvalidParameter(format!("blockhash must be hexadecimal string (not '{value}')"))
    })
}
...
mod tests {
    ...
    fn context_with_tip(
        ...
        context.chain.chain_network = network;
    ...
    fn getblockchaininfo_names_testnet4_separately_from_testnet3() {
        let mut context = Context::new();
        context.chain.chain_network = bitcoin_rs_primitives::Network::Testnet4;
...
mod chaintxstats_window_tests {
    ...
    fn a_log_that_does_not_reach_genesis_reports_no_count() {
        let ctx = chain_ctx(&TIMES);
        {
            ...
            let mut blocks = ctx.chain.blocks.write();
