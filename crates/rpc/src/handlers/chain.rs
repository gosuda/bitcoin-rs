...
pub(crate) fn gettxoutsetinfo(ctx: &Arc<Context>, params: &Value) -> Result<Value, RpcError> {
    ...
    let (stats, txouts, transactions, set_hash, tip_height, tip_hash, disk_size) =
        ctx.utxo.with_stable_view(|view| {
            // One published-tip read for the whole response: `height` and
            // `best_block` describe the tip as of this pinned snapshot, and
            // every scanned field below comes from the same stable view, so
            // a connect landing after the lock cannot mix two states into
            // one response.
            let tip = ctx.applied_tip.load_full();
            let tip_height = tip.as_ref().map_or(0, |tip| tip.height);
            let tip_hash = tip
                .as_ref()
                .map_or_else(|| ctx.chain_network.genesis_block_hash(), |tip| tip.hash);
