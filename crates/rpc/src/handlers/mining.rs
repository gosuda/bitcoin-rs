...
pub(crate) fn getblocktemplate(ctx: &Arc<Context>, params: &Value) -> Result<Value, RpcError> {
    let control = ctx
        .mining_control
        .as_ref()
        .ok_or(RpcError::MethodNotFound("getblocktemplate".to_owned()))?;
...
pub(crate) fn getmininginfo(ctx: &Arc<Context>, params: &Value) -> Result<Value, RpcError> {
    ...
    let control = ctx
        .mining_control
        .as_ref()
        .ok_or(RpcError::MethodNotFound("getmininginfo".to_owned()))?;
...
pub(crate) fn submitblock(ctx: &Arc<Context>, params: &Value) -> Result<Value, RpcError> {
    let control = ctx
        .mining_control
        .as_ref()
        .ok_or(RpcError::MethodNotFound("submitblock".to_owned()))?;
...
pub(crate) fn submitheader(ctx: &Arc<Context>, params: &Value) -> Result<Value, RpcError> {
    let control = ctx
        .mining_control
        .as_ref()
        .ok_or(RpcError::MethodNotFound("submitheader".to_owned()))?;
...
pub(crate) fn prioritisetransaction(ctx: &Arc<Context>, params: &Value) -> Result<Value, RpcError> {
    let (txid_str, fee_delta, dummy) = if let Some(array) = params.as_array() {
        ...
        let fee_delta = array
            ...
            .ok_or(RpcError::InvalidType("parameter must be an integer".to_owned()))?;
        (
        ...
        && !dummy.is_null()
    {
        ...
        let nonzero = dummy
            ...
            .ok_or(RpcError::InvalidType("dummy must be a number".to_owned()))?;
        if nonzero {
...
pub(crate) fn generatetoaddress(ctx: &Arc<Context>, params: &Value) -> Result<Value, RpcError> {
    ...
    let control = ctx
        .mining_control
        .as_ref()
        .ok_or(RpcError::MethodNotFound("generatetoaddress".to_owned()))?;
...
pub(crate) fn generateblock(ctx: &Arc<Context>, params: &Value) -> Result<Value, RpcError> {
    ...
    let control = ctx
        .mining_control
        .as_ref()
        .ok_or(RpcError::MethodNotFound("generateblock".to_owned()))?;
...
pub(crate) fn getnetworkhashps(ctx: &Arc<Context>, params: &Value) -> Result<Value, RpcError> {
    let control = ctx
        .mining_control
        .as_ref()
        .ok_or(RpcError::MethodNotFound("getnetworkhashps".to_owned()))?;
...
fn parse_generateblock_transactions(
    ...
    let Some(entries) = value.as_array() else {
        return Err(RpcError::InvalidType("transactions must be an array".to_owned()));
    };
...
fn parse_block_template_request(params: &Value) -> Result<BlockTemplateRequest, RpcError> {
    ...
    if !request.is_object() {
        return Err(RpcError::InvalidType("template request must be an object".to_owned()));
    }
        ...
        let data =
            request
                ...
                .ok_or(RpcError::InvalidType(
                    "Missing data String key for proposal".to_owned(),
                ))?;
    ...
    let long_poll_id = match request.get("longpollid") {
        ...
        Some(value) => {
            let Some(text) = value.as_str() else {
                return Err(RpcError::InvalidType("longpollid must be a string".to_owned()));
            };
