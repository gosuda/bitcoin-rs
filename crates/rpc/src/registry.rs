...
declare_rows! {
    ...
    "sendrawtransaction", SurfaceKind::Rpc, Status::Deviation, "", CORE_VERSION, "Replacement allows nonsignaling originals (crates/mempool/src/rbf.rs). See docs/policies/mempool-policy.md for Core 31.1 process evidence and the remaining new-input, eviction-count, and feerate-diagram differences under #639.", "0.4.0", Some(tx::sendrawtransaction);
    "testmempoolaccept", SurfaceKind::Rpc, Status::Deviation, "", CORE_VERSION, "Preview shares submission's signal-independent replacement evaluator (crates/mempool/src/rbf.rs). See docs/policies/mempool-policy.md for Core 31.1 process evidence and remaining #639 differences.", "0.4.0", Some(tx::testmempoolaccept);
    ...
    "getmempoolinfo", SurfaceKind::Rpc, Status::Deviation, "", CORE_VERSION, "Policy fields project the enforced MempoolPolicySnapshot (crates/mempool/src/policy.rs): fullrbf is true, matching Core 31.1's signal-independent replacement policy; limitclustercount and limitclustersize project the cluster limits admission enforces; optimal is always true because the fee-rate index is rewritten under the pool write lock (crates/rpc/src/handlers/mempool.rs).", "0.4.0", Some(mempool::getmempoolinfo);
    ...
    "getmininginfo", SurfaceKind::Rpc, Status::Implemented, "", CORE_VERSION, "Pinned v30 shape including bits/target and next-block facts. Unset currentblocktx, currentblockweight, and signet_challenge are omitted like Core.", "0.4.0", Some(mining::getmininginfo);
    ...
    "generatetoaddress", SurfaceKind::Rpc, Status::Implemented, "", CORE_VERSION, "Assembles, solves, and submits n blocks paying the given address through the mining coordinator. Invalid-output behavior is defined in docs/contracts/external-api.md#API-29.", "0.4.0", Some(mining::generatetoaddress);
    "generateblock", SurfaceKind::Rpc, Status::Implemented, "", CORE_VERSION, "Assembles and solves one block paying an address or descriptor from the listed mempool txids or raw txs in that order. See the canonical contract: docs/contracts/external-api.md#API-31.", "0.4.0", Some(mining::generateblock);
