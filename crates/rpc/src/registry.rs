...
declare_rows! {
    ...
    "generatetoaddress", SurfaceKind::Rpc, Status::Implemented, "", CORE_VERSION, "Assembles, solves, and submits n blocks paying the given address through the mining coordinator. See API-26 in docs/contracts/external-api.md for Core invalid-output behavior.", "0.4.0", Some(mining::generatetoaddress);
    "generateblock", SurfaceKind::Rpc, Status::Implemented, "", CORE_VERSION, "Assembles and solves one block paying an address or descriptor from the listed mempool txids or raw txs in that order; third param is Core's submit flag. Unknown 64-hex txids are -5; raw-tx decode failures are -22. See API-26 in docs/contracts/external-api.md for Core invalid-output behavior.", "0.4.0", Some(mining::generateblock);
