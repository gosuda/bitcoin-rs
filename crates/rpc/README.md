...
- **System Owners**:
  - **Routing & Demux**: `classify` in `crates/rpc/src/server.rs` owns the listener directory table; see [Transport Demuxing](#1-protocol-demuxing-and-authentication) above for the authoritative routing rules. The Esplora `/api` versus `/esplora` split is contract WF-02.
