...
- **System Owners**:
  - **Routing & Demux**: `classify` in `crates/rpc/src/server.rs` is the listener directory table: `GET /rest/*` → REST; `GET` otherwise → Esplora (`/api` public, `/esplora` backend, anything else 404s); `POST` under `/api` or `/esplora` → closed Esplora; other `POST` → JSON-RPC authentication.
