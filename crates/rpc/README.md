...
- **Transport Demuxing**: Private free function `classify` (`crates/rpc/src/server.rs`) is the listener directory table. `serve_connection` dispatches that table before authentication. The authoritative Esplora namespace, fallthrough, and surface rules are defined in [WF-02](../../docs/contracts/wallet-facing.md#wf-02-operations-a-wallet-actually-issues):
