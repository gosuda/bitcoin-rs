...
- **System Owners**:
  ...
  - **Status Reads vs Live Scans**: Published-status reads (`getblockchaininfo`, `getchaintxstats`, and the typed `Context::sync_progress`) project from one `AppliedView` capture of the last complete applied publication and never acquire the node's chain-transition mutex, so a block mid-transition cannot stall an explorer or monitoring client. Reads that must see a *mutable* chainstate as one piece (`scantxoutset`'s tip-plus-UTXO scan, and mempool admission) keep running inside `Context::with_stable_chainstate`, which holds that mutex across the whole read. The mutex is mutable-chainstate exclusion, not status synchronization.
