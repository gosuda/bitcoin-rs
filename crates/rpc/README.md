...
- **Transport Demuxing**: Private free function `classify` (`crates/rpc/src/server.rs`) owns the listener directory dispatch, and `serve_connection` applies it before authentication. The authoritative Esplora namespace, fallthrough, and surface rules are defined in [WF-02](../../docs/contracts/wallet-facing.md#wf-02-operations-a-wallet-actually-issues).
  - JSON-RPC authentication is handled by `Auth::validate_header` (`crates/rpc/src/auth.rs`).
