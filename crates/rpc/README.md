...
- **System Owners**:
  ...
  - **Status Reads vs Live Scans**: Published-status reads (`getblockchaininfo`, `getchaintxstats`, and the typed `Context::sync_progress`) project from one `AppliedView` capture of the last complete applied publication and never acquire the node's chain-transition mutex, so a block mid-transition cannot stall an explorer or monitoring client. Reads that must see a *mutable* chainstate as one piece (`scantxoutset`'s tip-plus-UTXO scan, `gettxoutsetinfo`, and REST `getutxos`) run inside `Context::with_stable_chainstate`, which holds that mutex across the whole read. Mempool admission is not one of them: `Context::admit_transaction` submits through `MempoolGateway::submit_transaction` under the mempool's optimistic `AdmissionFence::Stable` version fence, retrying when a transition lands mid-check rather than excluding one. The mutex is mutable-chainstate exclusion, not status synchronization.
