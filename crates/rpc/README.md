...
- **Transport Demuxing**: Private free function `classify` (`crates/rpc/src/server.rs`) is the listener directory table. `serve_connection` dispatches that table before authentication:
  The listener routing rules are the canonical `WF-02` contract in
  `docs/contracts/wallet-facing.md`; `classify` implements that contract.
  Keep route ownership there rather than duplicating the directory table in
  this subsystem README.
