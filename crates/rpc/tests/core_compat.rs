...
use bitcoin_rs_mining::{FakeMiningControl, MiningCapability, TemplateMutation};
use bitcoin_rs_primitives::{Hash256, Network, client_version};
...
fn mining_responses_deserialize_into_pinned_types() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let handler = Handler::new(Arc::new(ctx.with_mining_control(
        FakeMiningControl::with_template(
            mining_fixture::canned_template(
                vec![MiningCapability::new("coinbasetxn")],
                vec![
