...
The crate owns the domain `Candidate` and the node-facing mining contract
([`MiningControl`](crate::MiningControl), [`BlockTemplate`](crate::BlockTemplate),
[`MiningInfo`](crate::MiningInfo)). BIP22/BIP23 JSON projection lives in RPC.
Long-poll waiting and block submission live in the node-owned coordinator
that implements `MiningControl`.
