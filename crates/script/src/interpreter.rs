...
impl Interpreter {
    ...
    /// [`Self::execute_with_prevouts`] with a caller-owned sighash-cache
    /// slot: when the spending transaction is used unmodified (the serial
    /// per-transaction path in `verify_transaction`), one cache serves every
    /// input of that transaction, so the BIP143/BIP341 aggregates are
    /// computed once. The parallel per-input block path passes a fresh slot
    /// per input: same-transaction inputs verify concurrently there and
    /// cannot share a `&mut` cache without serializing the hashing.
    ///
    /// A grafted input (window verification with replaced script or witness)
    /// hashes a different transaction image, so the shared cache cannot be
    /// reused there; this entry point discards the slot for grafted inputs
    /// rather than computing wrong hashes.
    /// Nine parameters mirror [`Self::execute_with_prevouts`] plus the
    /// cache slot; grouping them would diverge from the sibling entry point.
    #[allow(clippy::too_many_arguments, clippy::needless_lifetimes)]
    pub fn execute_with_shared_cache<'t>(
        &self,
        script_pubkey: &[u8],
        script_sig: &[u8],
        witness: &[Vec<u8>],
        ...
        tx: &'t Tx,
        input_idx: usize,
        shared: &mut Option<SighashCache<'t>>,
    ) -> Result<bool, ScriptError> {
        let input = tx
            .inputs
            .get(input_idx)
        ...
        if !matches_tx {
            shared.take();
            return self.execute_with_prevouts(
                script_pubkey,
                script_sig,
                witness,
                flags,
        ...
        let prevout = prevouts
            ...
            .ok_or(ScriptError::TaprootPrevoutsUnavailable)?;
        if is_p2tr(script_pubkey) && flags.contains(VerifyFlags::TAPROOT) {
            // The taproot fast path builds its own per-call hasher today;
            // the shared slot stays untouched for the remaining inputs.
            return verify_taproot(tx, input_idx, script_pubkey, witness, prevouts, flags);
        }
        let cache = shared.take().unwrap_or_else(|| SighashCache::new(tx));
        let mut checker =
            TxSignatureChecker::new_with_cache(tx, input_idx, prevout.value, prevouts, cache);
        let verdict = verify_script(
            script_sig,
            script_pubkey,
            witness,
            flags.filled(),
        ...
        *shared = Some(checker.into_cache());
        verdict?;
        Ok(true)
    }
}
