...
impl InitialBlockDownload {
    ...
    pub fn is_active(&self, now: u64) -> bool {
        ...
        if self.still_active(now) {
            // A concurrent caller may have latched "left" while the
    ...
    /// The unlocked predicate behind [`Self::is_active`]: minimum work, then
    /// tip freshness against the caller's clock.
