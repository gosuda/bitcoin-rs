//! CONTRACT: docs/contracts/recovery.md#RCV-04; CONSTRAINTS.md#CL-14.
//!
...
fn write_failure_requires_recovery_before_serving_or_retrying() {
    for mode in [
        ...
        assert!(matches!(
            coins.connect_block(&block(vec![], vec![]), &a, mode),
            Err(PersistentUtxoError::RecoveryRequired)
        ));
        assert!(matches!(
            coins.undo_block(&UndoBatch::default(), mode),
            Err(PersistentUtxoError::RecoveryRequired)
        ));
        assert!(matches!(
            coins.flush(),
...
/// CL-14: read refills enforce the same budget as write refills.
#[test]
...
/// CL-14: eviction retains pinned candidates until flush releases them.
#[test]
