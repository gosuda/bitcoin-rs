...
outpoint after an eight-byte script-hash prefix, with `vout` packed to 3 bytes.
...
range. Confirmed prefix keys store height big-endian so lexicographic order is
numeric. `TxPosition` is a packed 6-byte (`u24` offset + `u24` length) locator.
`PreparedBlock`/`PreparedBatch` under `PreparedBatchLimits` bound one atomic
