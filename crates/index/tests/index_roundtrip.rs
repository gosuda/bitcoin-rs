...
#[test]
fn format_4_open_resets_all_derived_capabilities() -> Result<(), Box<dyn std::error::Error>> {
    let store = Arc::new(MemoryStore::default());
    ...
    assert!(store.count(ColumnFamily::BlockHeaders) > 0);
...
fn prepare_block_verifies_header_identity_and_parent() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert_eq!(block.encoded_bytes, 68);
...
fn batch_caps_admit_oversized_first_block() -> Result<(), Box<dyn std::error::Error>> {
    ...
    assert!(batch.try_push(block1).is_err());
    assert_eq!(batch.len(), 1);
    assert_eq!(batch.encoded_bytes(), 68);
    assert!(batch.is_full());
    assert_eq!(
        batch.watermark(),
        Some(IndexWatermark {
