...
/// Contract sources: `crates/node/src/checkpoint/headers.rs` owns the
/// `headers-v1` prefix layout (`HEADER_MAGIC`, `HEADER_VERSION`, and
/// `HEADER_PREFIX_LEN`) and its exact-length reader; the corruption and
/// recovery policy is documented in `docs/contracts/recovery.md` ("Recovery
/// contract", especially the checkpoint-independence requirement). Keep the
/// byte offsets below tied to that codec owner rather than treating them as
/// incidental fixture details.
#[test]
