...
use bitcoin_rs_mempool::{
    AdmissionOrigin, MempoolObserver, MutationEnvelope, MutationOutcome, RemovalReason,
};
...
/// Installed on the node-owned gateway at [`crate::state::NodeState::open`]
/// so every block-connect and reorg mutation — the apply path already
/// publishes both through the gateway — drops the cache without giving
/// `apply.rs` a second owner of admission state.
