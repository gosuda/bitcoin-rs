...
use bitcoin_rs_p2p::{
    DrainedBlock, InboundBlock, InboundHeaders, Message, PeerInfo, PeerSource, PeerTable,
    StagedBlock, SyncAction, SyncDisconnectReason, SyncPlanner,
};
...
/// `P2pService::select_download_peers`. The P2P service does not hold a second
/// window.
...
mod tests {
    ...
    -> Result<(), Box<dyn std::error::Error>> {
        ...
        assert!(sync.handles.mempool_gateway.stable_generation().is_some());
        assert!(!sync.planner.lock().stager().contains(&fork_hash));
        assert!(!sync.planner.lock().stager().contains(&descendant_hash));
        assert_eq!(sync.planner.lock().window().received_len(), 0);
        Ok(())
    }
    ...
    fn fatal_settlement_halts_further_apply_attempts() -> Result<(), Box<dyn std::error::Error>> {
        let (sync, _peers, _applied_tip, main, _blocks_tx) = sync_with_mined_chain(1)?;
        stage_body(&sync, &main[0]);
        let staged = sync.planner.lock().stager().received_len();
        assert!(
            staged > 0,
        ...
        assert_eq!(
            sync.planner.lock().stager().received_len(),
            staged,
