...
mod capability_revision_tests {
    ...
    // IDX-02: production status exposes the existing runtime wake counter.
    #[test]
    fn capability_revision_tracks_runtime_wakes() {
        let (wake_tx, _wake_rx) = crossbeam_channel::bounded(1);
        let runtime = Arc::new(TxIndexRuntime::new(wake_tx));
        let source = TxIndexCapability::new(
            None,
        ...
        runtime.publish_failed("test failure");
        assert_eq!(source.revision(), 1, "not a lifecycle version");
    }
}
