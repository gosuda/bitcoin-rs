...
use bitcoin::{hashes::Hash as _, p2p::message_blockdata::GetHeadersMessage};

use bitcoin_rs_p2p::{InboundHeaders, Message, PeerSource, download_window::SyncPeer};
...
use super::{
    BlockSync,
    peers::{active_demonstrated_height, is_peer_fault, outranks, sync_peer_candidate},
};

/// Maximum number of locator entries we ever send.
pub(super) const LOCATOR_MAX_ENTRIES: usize = 32;

/// Wire protocol version we advertise on outbound `getheaders`.
pub(super) const PROTOCOL_VERSION: u32 = 70_016;

/// Time after which an unanswered `getheaders` request may be retried.
pub(super) const HEADER_REQUEST_TIMEOUT: Duration = Duration::from_secs(5);

#[derive(Clone, Copy, Debug)]
pub(super) struct PendingHeaderRequest {
    pub(super) peer_addr: SocketAddr,
    pub(super) locator_tip_hash: Hash256,
